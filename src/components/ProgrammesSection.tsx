...
const ProgrammesSection = () => {
  return <section className="bg-gradient-to-b from-white via-secondary/30 to-white py-20 overflow-hidden">
      <div className="container mx-auto px-6">
        ...
        <div className="mb-16">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {whatsappGroups.map((group, index) => <a key={index} href={group.link} target="_blank" rel="noopener noreferrer" className="group relative bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100">
                {group.badge && <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-green-500 text-white text-xs font-semibold px-3 py-1 rounded-full">
                    {group.badge}
                  </span>}
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-12">
          {programmes.filter(p => p.featured).map((programme, index) => <div key={index} className="group relative bg-white rounded-3xl p-8 shadow-lg hover:shadow-2xl transition-all duration-500 border border-primary/10 overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-wrap gap-3">
                  {programme.link && <Button className="bg-primary hover:bg-primary-dark text-white shadow-lg hover:shadow-xl transition-all group/btn" onClick={() => window.open(programme.link, '_blank')}>
                      Learn More
                  ...
                  {programme.medicalLink && programme.dentalLink && <>
                      <Button className="bg-primary hover:bg-primary-dark text-white shadow-lg hover:shadow-xl transition-all" onClick={() => window.open(programme.medicalLink, '_blank')}>
                        Medical
                        <ExternalLink className="ml-2 h-4 w-4" />
                      </Button>
                      <Button variant="outline" className="border-2 border-primary text-primary hover:bg-primary hover:text-white transition-all" onClick={() => window.open(programme.dentalLink, '_blank')}>
                        Dental
                        <ExternalLink className="ml-2 h-4 w-4" />
                      </Button>
                    </>}
        ...
        {programmes.filter(p => !p.featured).map((programme, index) => <div key={index} className="group bg-gradient-to-r from-primary to-primary-dark rounded-3xl p-8 mb-16 text-white relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="bg-white/20 backdrop-blur-sm px-6 py-3 rounded-full">
                <span className="font-semibold text-white">2026 Intake Soon!</span>
              </div>
            </div>
          </div>)}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {bespokeServices.map((service, index) => <div key={index} className="group bg-white rounded-2xl p-6 border border-primary/10 hover:border-primary/30 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 text-center">
              ...
              <Button variant="ghost" size="sm" className="text-primary hover:bg-primary/10 group-hover:bg-primary group-hover:text-white transition-all" onClick={() => window.open(service.link, '_blank')}>
                Learn More
